                ...
                <div class="flex-1 flex flex-col p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="flex gap-2 items-center">
                            <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold <?php echo $item['badgeColor']; ?> border border-transparent shadow-sm">
                                <?php if ($item['icon'] === 'document-text'): ?><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 4H7a2 2 0 01-2-2V6a2 2 0 012-2h7l5 5v11a2 2 0 01-2 2z" /></svg><?php endif; ?>
                                <?php if ($item['icon'] === 'cpu'): ?><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 9h6v6H9z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 3v2m0 14v2m7-7h-2M5 12H3m15.364-6.364l-1.414 1.414M6.343 17.657l-1.414 1.414m12.728 0l-1.414-1.414M6.343 6.343L4.929 4.929" /></svg><?php endif; ?>
                                <?php if ($item['icon'] === 'user'): ?><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.121 17.804A9 9 0 1112 21a9 9 0 01-6.879-3.196z" /></svg><?php endif; ?>
                                <?php echo $item['badge']; ?>
                            </span>
                            <?php if ($is_published): ?>
                    ...
                    <div class="flex-1 flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg mb-5 min-h-[220px] max-h-[400px] overflow-hidden border border-gray-200 shadow-inner">
                        <?php if ($resume_url): ?>
                            <iframe id="resume-preview-<?php echo $resume->ID; ?>" src="<?php echo esc_url($resume_url); ?>" class="w-full h-[300px] border-0 rounded" loading="lazy" onload="injectTailwindStyles(this)" onerror="this.parentNode.innerHTML='<div class=\'flex flex-col items-center justify-center w-full h-full text-gray-400 text-sm\'><svg xmlns=\'http://www.w3.org/2000/svg\' class=\'h-8 w-8 mb-2\' fill=\'none\' viewBox=\'0 0 24 24\' stroke=\'currentColor\'><path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M6 18L18 6M6 6l12 12\' /></svg>Unable to preview this resume.</div>';" ></iframe>
                        <?php else: ?>
                            <div class="flex flex-col items-center justify-center w-full h-full text-gray-400 text-sm py-10">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2a4 4 0 018 0v2m-4-4a4 4 0 100-8 4 4 0 000 8z" /></svg>
                    ...
                    <div class="flex gap-3 mt-auto">
                        <?php if ($resume_url): ?>
                            <a href="<?php echo esc_url($resume_url); ?>" download class="inline-flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold border border-gray-300 bg-white text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition shadow-sm">
                        ...
                        <?php if ($resume_url && !$is_published): ?>
                            <button type="button" onclick="publishResume(<?php echo $resume->ID; ?>)" class="inline-flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-semibold border border-transparent bg-blue-600 text-white hover:bg-blue-700 transition shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400">
